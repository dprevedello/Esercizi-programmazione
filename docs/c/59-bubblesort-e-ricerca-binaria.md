...
La ricerca binaria può essere implementata in modo **iterativo** (con un ciclo `while` e due indici `sinistra` e `destra`) oppure **ricorsivo**.
...
- **Ottimizzazione:** puoi aggiungere una variabile `scambiato` per interrompere il ciclo esterno se una passata non ha prodotto scambi (ottimizzazione).
- Implementa la ricerca binaria con un ciclo `while (sinistra <= destra)`.
