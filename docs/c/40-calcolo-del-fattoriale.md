...
Il calcolo si esegue con un ciclo `for` che moltiplica progressivamente i valori da 1 fino a n. Poiché il fattoriale cresce molto rapidamente (già 13! supera il massimo di un `int` a 32 bit), è opportuno usare il tipo `long long` per memorizzare il risultato.
